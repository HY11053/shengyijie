                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($articles as $article)
                            <tr>
                                ...
                                <td>@if(\Carbon\Carbon::now() > \Carbon\Carbon::parse($article->created_at)->addDays(7)) @if($article->ismake) {{$article->created_at}} @else 未审核 @endif @else @if($article->ismake) {{\Carbon\Carbon::parse($article->published_at)->diffForHumans()}} @else 未审核 @endif @endif </td>
                                <td>{{$article->write}}</td>
                                <td>{{$article->click}}</td>
