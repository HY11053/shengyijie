                ...
                <div class="box-body">
                    <table class="table table-bordered table-striped  table-hover">
                        ...
                        <tr>
                           ...
                           <td>
                               @php
                                   preg_match('#\/news\/(\d+)#',$adminlist->host,$matches);
                                   if (isset($matches[1]))
                                   {
                                   ...
                                   $isnew=\App\AdminModel\Archive::where('id',$matches[1])->value('created_at')>\Carbon\Carbon::now()->subMonth()?'<i class="fa  fa-fire"></i>':'';
                                   }elseif(preg_match('#\/zhishi\/(\d+)#',$adminlist->host,$matches)){
                                       if (isset($matches[1]))
                                       {
                                       $thisartcileinfos=\App\AdminModel\KnowedgeNew::where('id',$matches[1])->first(['id','brandid','created_at','typeid']);
                                       $thisbrandartcileinfos=\App\AdminModel\Brandarticle::where('id',$thisartcileinfos->brandid)->first(['id','brandname','typeid']);
                                       if ($thisartcileinfos->brandid && isset($thisbrandartcileinfos->brandname))
                                       {
                                        $brand=$thisbrandartcileinfos->brandname;
                                       ...
                                       $isnew=\App\AdminModel\KnowedgeNew::where('id',$matches[1])->value('created_at')>\Carbon\Carbon::now()->subMonth()?'<i class="fa  fa-fire"></i>':'';
                                       }
                                   }else{
                                       preg_match('#\/xm\/(\d+)#',$adminlist->host,$matches);
                                       if (isset($matches[1])){
