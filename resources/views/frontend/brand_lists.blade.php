...
@section('main')
    <div class="box">
        <div class="bk weizhi_locations" style="margin-top:4px;"><span><a href="/">{{config('app.indexname')}}</a>&gt;<a class="dq">{{$thisTypeinfos->typename}}</a></span></div>
        <div class="search_k">
            ...
            <div class="xuanze clearfix">
                ...
                <span @if(!$level) class=dq @endif>@if($province)<a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}?province={{array_search($province,$arealists->toArray())}}">不限</a> @else <a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}">不限</a> @endif </span>
                <ul>
                    @foreach($investmentlists as $id=>$investmentlist)
                        @if($province)
                            <li @if(array_search($level,$investmentlists->toArray())==$id ) class="dq" @endif><a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}?level={{$id}}&province={{array_search($province,$arealists->toArray())}}">{{$investmentlist}}</a></li>
                        @else
                            <li @if(array_search($level,$investmentlists->toArray())==$id ) class="dq" @endif><a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}?level={{$id}}">{{$investmentlist}}</a></li>
                        @endif
                    @endforeach
                </ul>
            </div>
            ...
            <div class="xuanzea clearfix">
                ...
                <span  @if(!$province) class=dq @endif >@if($level)  <a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}?level={{array_search($level,$investmentlists->toArray())}}">全部</a> @else <a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}">全部</a> @endif </span>
                <ul>
                    @foreach($arealists as $areaid=>$arealist)
                        @if($level)
                            <li @if(array_search($province,$arealists->toArray())==$areaid ) class="dq" @endif><a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}?level={{array_search($level,$investmentlists->toArray())}}&province={{$areaid}}">{{$arealist}}</a></li>
                        @else
                            <li @if(array_search($province,$arealists->toArray())==$areaid ) class="dq" @endif><a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}?province={{$areaid}}">{{$arealist}}</a></li>
                        @endif
                    @endforeach
                </ul>
                    ...
                    @foreach($brands as $brand)
                        <div class="pinpai_h">
                            ...
                            <div class="pinpai_h_c">
                                <div class="pp_bt"><strong><a href="/xm/{{$brand->id}}" title="{{$brand->brandname}}" target="_blank">{{$brand->brandname}}</a></strong>[<a href="/{{$thisTypeTopInfo->real_path}}">{{$thisTypeTopInfo->typename}}</a> > <a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeinfos->id}}/">{{$thisTypeinfos->typename}}</a>]</div>
                ...
                @else
                    @foreach($tuijianbrands as $brand)
                        <div class="pinpai_h">
                            <div class="pinpai_h_pic"><span><a href="/xm/{{$brand->id}}" title="{{$brand->brandname}}" target="_blank"><img src="{{$brand->litpic}}" alt="{{$brand->brandname}}" /></a></span></div>
                            <div class="pinpai_h_c">
                                <div class="pp_bt"><strong><a href="/xm/{{$brand->id}}" title="{{$brand->brandname}}" target="_blank">{{$brand->brandname}}</a></strong>[<a href="/{{$thisTypeTopInfo->real_path}}">{{$thisTypeTopInfo->typename}}</a> > <a href="/{{$thisTypeTopInfo->real_path}}/{{$thisTypeTopInfo->id}}">{{$thisTypeTopInfo->typename}}</a>]</div>
                                <span><em><strong>合作模式：{{$brand->brandmoshi}}</strong></em><strong>品牌总部：</strong>{{$brand->brandorigin}}</span>
                                <span><strong>运营机构：</strong>{{$brand->brandgroup}}</span>
                                <p>{{$brand->description}}...</p>
                            </div>
